        ...
        return modifiers
            ...
            .map(function (modifier) {
                var match = modifier.match(converter.modifierRegExp),
                    screenSize = match[1],
                    units = match[2],
                    className = util.format(converter.classNameTemplate, screenSize);

                if (converter.renderPropertyName)
                    className = util.format(className, propertyName);

                return util.format(className, units);
            });
    };
}

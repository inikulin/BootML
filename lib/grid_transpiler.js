...
function buildAttrConverterMethods(converter) {
    ...
    converter.toClassNames = function (attrName) {
        ...
        if (converter.compoundParts) {
            var parts = attrMatch[2];

            classNames = parts
                .trim()
                .split('-')
                .filter(function (part) {
                    return converter.compoundParts.re.test(part);
                })
                .map(function (part) {
                    var partMatch = part.match(converter.compoundParts.re);

                    return renderTemplateWithRegExpMatch(
                        converter.classNameTemplate,
                        converter.compoundParts.reGroupNames,
                        partMatch
                    );
                })
        }

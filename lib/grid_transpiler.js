...
GridTranspiler.prototype._adjustClassNameList = function (hasSizeAttrs, sizeClassName, attrClassNames) {
    //NOTE: since we've traversed attributes in reverse order we need to reverse
    //resulting class name list
    attrClassNames = attrClassNames.reverse();
...
GridTranspiler.prototype._convertColAttrsToClassNames = function (attrs, sizeClassName) {
    ...
    for (var i = attrs.length - 1; i >= 0; i--) {
        for (var j = 0; j < ColumnAttrConverters.length; j++) {
            if (ColumnAttrConverters[j].condition(attrs[i])) {
                var converted = ColumnAttrConverters[j].toClassNames(attrs[i]);

                hasSizeAttrs |= ColumnAttrConverters[j].isSizeAttr;
                attrClassNames = attrClassNames.concat(converted);
                attrs.splice(i, 1);
    ...
    return this._adjustClassNameList(hasSizeAttrs, sizeClassName, attrClassNames);
};

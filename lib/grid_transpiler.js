...
var COL_TAG = 'c',
    ...
    NUMBERED_OFFSET_CLASS_TEMPLATE = 'col-md-offset-%s',
    OFFSET_CLASS_TEMPLATE = 'col-%s-offset-%s';

//Column attributes
var SIZE_MODIFIER_REGEXP = /^(xs|sm|md|lg)([1-9]|1[0-2])$/,
    OFFSET_MODIFIER_REGEXP = /^(xs|sm|md|lg)([0-9]|1[0-2])$/,
    NUMBERED_OFFSET_ATTR_REGEXP = /^offset([0-9]|1[0-2])$/,
    COMPOUND_ATTR_MODIFIER_SEPARATOR = '-',
    COMPOUND_SIZE_ATTR_PREFIX_REGEXP = /^size:(\S+)$/,
    COMPOUND_OFFSET_ATTR_PREFIX_REGEXP = /^offset:(\S+)$/;
...
function buildCompoundAttrConverterMethods(converter) {
    ...
    converter.toClassNames = function (attrName) {
        var match = attrName.match(converter.prefixRegExp);
        ...
        return match[1]
            ...
            .map(function (modifier) {
                ...
                return util.format(converter.classNameTemplate, match[1], match[2]);
            });
    };
}
...
var ColumnAttrConverters = [
    ...
    {
        name: 'Numbered offset attribute',
        isSizeAttr: false,
        isCompound: false,
        attrRegExp: NUMBERED_OFFSET_ATTR_REGEXP,
        classNameTemplate: NUMBERED_OFFSET_CLASS_TEMPLATE
    },

    {
        name: 'Compound offset attribute',
        isSizeAttr: false,
        isCompound: true,
        prefixRegExp: COMPOUND_OFFSET_ATTR_PREFIX_REGEXP,
        modifierRegExp: OFFSET_MODIFIER_REGEXP,
        classNameTemplate: OFFSET_CLASS_TEMPLATE
    }
];

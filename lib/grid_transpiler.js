...
var SIZE_MODIFIER_REGEXP = /^(xs|sm|md|lg)([1-9]|1[0-2])$/,
    OFFSET_MODIFIER_REGEXP = /^(xs|sm|md|lg)([0-9]|1[0-2])$/,
...
//-------------------------------------------------===------------------------------------------------------
function convertCompoundAttr(attrName, prefixRegExp, modifierRegExp, classNameTemplate) {
    var match = attrName.match(prefixRegExp);

    return match[1]
        .trim()
        .split(COMPOUND_ATTR_MODIFIER_SEPARATOR)
        .filter(function (item) {
            return modifierRegExp.test(item);
        })
        .map(function (modifier) {
            var match = modifier.match(modifierRegExp);
            return util.format(classNameTemplate, match[1], match[2]);
        });
}
...
var ColumnAttrConverters = [
    {
        ...
        condition: function (attrName) {
            return SIZE_MODIFIER_REGEXP.test(attrName);
        },

        toClassNames: function (attrName) {
            var match = attrName.match(SIZE_MODIFIER_REGEXP);
            return [util.format(COL_SIZE_CLASS_TEMPLATE, match[1], match[2])];
        }
    },
    {
        ...
        condition: function (attrName) {
            return COMPOUND_SIZE_ATTR_PREFIX_REGEXP.test(attrName);
        },

        toClassNames: function (attrName) {
            return convertCompoundAttr(
                attrName,
                COMPOUND_SIZE_ATTR_PREFIX_REGEXP,
                SIZE_MODIFIER_REGEXP,
                COL_SIZE_CLASS_TEMPLATE
            )
        }

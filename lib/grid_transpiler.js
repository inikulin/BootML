...
function buildCompoundAttrConverterMethods(converter) {
    converter.condition = function (attrName) {
        return converter.prefix.re.test(attrName);
    };

    converter.toClassNames = function (attrName) {
        var prefixMatch = attrName.match(converter.prefix.re),
            modifiers = prefixMatch[2];
        ...
        return modifiers
            ...
            .map(function (modifier) {
                var match = modifier.match(converter.modifier.re),
                    className = renderTemplateWithRegExpMatch(
                        converter.classNameTemplate,
                        converter.prefix.reGroupNames,
                        prefixMatch
                    );
...
function buildAttrConverterMethods(converter) {
    ...
    converter.toClassNames = function (attrName) {
        var match = attrName.match(converter.attr.re);

        return [renderTemplateWithRegExpMatch(
            converter.classNameTemplate,
            converter.attr.reGroupNames,
            match
        )];
    };
}
